...
with st.sidebar:
    ...
    selected_signal = st.selectbox('Provided Signals', ['Generate A Random Signal', 'EKG Sample Signal', 'ECG Sample Signal', 'EMG Sample Signal', 'EEG Sample Signal', 'Generate sine ', 'remove','Provide A Local File Signal'])
...
def initialize_plot(fig):
    ...
    paper_bgcolor="rgba(2, 2, 46, 0.925)",
    font_color="white",
)
...
def add_to_plot(fig,x,y,name):
     if SNR==0 :
        ...
        noised_signal=createNoise(SNR,y)
        fig.add_trace(
        go.Scatter(x=x, y=y, name=name)
    )
...
if selected_signal == "Generate A Random Signal":
    ...
    genrate_button=st.button('genrate',key=0)
    if genrate_button:
    ...
    if st.session_state.list_of_signals:
        ...
        delete_button=st.button('delete',key=1,on_click=delete,args= (selected_value,))
    with col1:
